...
class ActorNetwork(object):
    def __init__(self, sess, state_dim, action_dim, action_bound, method_name,
        ...
        if method_name.lower() == "sgd":
            ...
            self.optimize = tf.train.AdagradOptimizer(learning_rate=self.learning_rate).\
                    apply_gradients(zip(self.mean_grads, self.mean_params))
        elif method_name.lower() == "rmsprop":
            self.optimize = tf.train.RMSPropOptimizer(learning_rate=self.learning_rate).\
                    apply_gradients(zip(self.mean_grads, self.mean_params))
